...
function endpoint2($requestString, $productName, $memory)
{
    //Shields.io
    $request = strtoupper($requestString);
    $cache = $memory->get("jomcraftapi.$productName.endpoint2.$request");
    ...
    if ($cache) {
        ...
        if ($product[$request]) {
            try {
                $version = json_decode($product[$request]);
                $versionV = ucfirst(strtolower($request));
                $arr = array('schemaVersion' => 1, 'label' => "Latest $versionV", 'message' => $version->latest, 'color' => '8cba05', 'cacheSeconds' => 3600);
                $returnJSON = json_encode($arr);
                $memory->set("jomcraftapi.$productName.endpoint2.$request", $returnJSON, time() + 7200);
